...
func menuIncidencias(scanner *bufio.Scanner, im *IncidenciaManager, vm *VehiculoManager) {
	for {
		...
		if opcion == "1" {
			...
			scanner.Scan()
			tipoStr := strings.ToLower(scanner.Text())
			...
			case "carroceria":
				tipo = Carroceria
			default:
				fmt.Println("Tipo no válido. Usando 'mecanica' por defecto.")
				tipo = Mecanica
			}

			fmt.Print("Prioridad (baja/media/alta): ")
			scanner.Scan()
			prioridadStr := strings.ToLower(scanner.Text())

			var prioridad Prioridad
			switch prioridadStr {
			case "baja":
				prioridad = PrioridadBaja
			case "media":
				prioridad = PrioridadMedia
			case "alta":
				prioridad = PrioridadAlta
			default:
				fmt.Println("Prioridad no válida. Usando 'media' por defecto.")
				prioridad = PrioridadMedia
			}
			...
			for i := 0; i < len(incidencias); i++ {
				fmt.Printf("ID: %d | VehículoID: %d | Tipo: %s | Prioridad: %d | Estado: %s | Descripción: %s\n",
			...
			scanner.Scan()
			estadoStr := strings.ToLower(scanner.Text())
...
func menuMecanicos(scanner *bufio.Scanner, mm *MecanicoManager, taller *Taller) {
	for {
		...
		if opcion == "1" {
			...
			scanner.Scan()
			espStr := strings.ToLower(scanner.Text())

			var especialidad Especialidad
			switch espStr {
			case "mecanica":
				especialidad = EspMecanica
			case "electrica":
				especialidad = EspElectrica
			case "carroceria":
				especialidad = EspCarroceria
			default:
				fmt.Println("Especialidad no válida. Usando 'mecanica' por defecto.")
				especialidad = EspMecanica
			}
